...
\begin{tikzpicture}[remember picture, overlay]
  ...
  % Informations
  \node[anchor=north west, font=\large, text width=12cm, thgrisfonce] at ([xshift=2.5cm, yshift=-15.2cm]current page.north west) {
    \begin{tabular}{ll}
    \textbf{Référence} & TH-AO-2024-BOOM-001 \\[4pt]
    \textbf{Version} & 1.0 \\[4pt]
    \textbf{Date} & 15 novembre 2024 \\[4pt]
    \textbf{Classification} & Confidentiel \\[4pt]
    \textbf{Rédacteurs} & Julien Dubois, Sophie Laurent \\[4pt]
    \textbf{Validation} & Antoine Mercier, Directeur Général
    \end{tabular}
  };
  % Pied de page
...
\centering
\begin{tikzpicture}[
  every node/.style={draw, rounded corners, minimum width=3.2cm, minimum height=1cm, align=center, font=\small},
  level 1/.style={sibling distance=4.5cm},
  edge from parent/.style={draw, -Stealth, thick, thbleu},
  dir/.style={fill=thbleu, text=white, font=\small\bfseries},
  chef/.style={fill=thorange, text=white, font=\small\bfseries},
  tech/.style={fill=thbleu!15, text=thbleu},
]
  \node[dir] (dg) {Antoine Mercier\\Directeur Général}
    child {node[dir] (dt) {Sophie Laurent\\Directrice Technique}
      child {node[chef] (cp) {Julien Dubois\\Chef de Projet}}
    };
  \node[tech, below left=1.5cm and -0.5cm of cp] (rs) {Thomas Bernard\\Resp. Réseau};
  \node[tech, below=1.5cm of cp] (is) {Camille Martin\\Ing. Systèmes};
  \node[tech, below right=1.5cm and -0.5cm of cp] (it) {Marc Lefèvre\\Ing. Télécom};
  \node[tech, below=1.2cm of rs] (t1) {3 Techniciens\\Déploiement};
  \node[tech, below=1.2cm of is] (t2) {2 Techniciens\\Systèmes};
  \node[tech, below=1.2cm of it] (t3) {2 Techniciens\\Câblage};
  \draw[-Stealth, thick, thbleu] (cp) -- (rs);
  \draw[-Stealth, thick, thbleu] (cp) -- (is);
  \draw[-Stealth, thick, thbleu] (cp) -- (it);
  \draw[-Stealth, thick, thbleu] (rs) -- (t1);
  \draw[-Stealth, thick, thbleu] (is) -- (t2);
  \draw[-Stealth, thick, thbleu] (it) -- (t3);
\end{tikzpicture}
...
\textbf{Thomas Bernard -- Responsable Réseau}

CCNP Entreprise et certifié Fortinet NSE7, Thomas cumule dix ans d'expérience en conception et administration de réseaux d'entreprise. Il a conçu l'architecture WAN SD-WAN de ChimExpress (25 sites) et maîtrise les environnements multi-sites à contraintes de disponibilité élevées.

\textbf{Camille Martin -- Ingénieure Systèmes}

Certifiée VMware VCP-DCV et ayant une solide expérience sur les environnements IBM Power (AS/400, IBM~i), Camille est responsable de la conception de l'infrastructure de virtualisation et de stockage. Elle a mené la migration d'un parc de 200 machines virtuelles vers une architecture hyperconvergée pour le compte de FroidLogistic.

\textbf{Marc Lefèvre -- Ingénieur Télécom}

Spécialiste des solutions de téléphonie sur IP et de communications unifiées, Marc est certifié 3CX Advanced et possède une expertise reconnue sur les déploiements DECT en environnement industriel. Il a déployé la téléphonie de la plateforme logistique de TransNord (600 postes, couverture DECT entrepôt).
...
\centering
\small
\begin{tabularx}{\textwidth}{L{2.8cm}L{2.2cm}L{4cm}L{2cm}R{2cm}}
\toprule
...
ToshiHaut applique une méthodologie projet éprouvée, inspirée du référentiel PRINCE2 et adaptée aux spécificités des déploiements d'infrastructure. Notre approche se structure en cinq phases :
...
Notre certification ISO~27001:2022 garantit par ailleurs que les données et informations de BOOM seront manipulées dans le respect des meilleures pratiques de sécurité de l'information tout au long du projet.
...
\begin{table}[H]
\centering
\begin{tabularx}{\textwidth}{L{3cm}C{1.5cm}L{4cm}L{5cm}}
\toprule
...
\centering
\small
\begin{tabularx}{\textwidth}{L{2.5cm}C{1.2cm}L{3.5cm}L{5.5cm}}
\toprule
...
L'analyse du cahier des charges et de l'environnement de BOOM nous permet d'identifier cinq enjeux majeurs que notre proposition technique s'attache à adresser :
...
Pour le plan de reprise d'activité, nous avons sélectionné le datacenter SFR Netcenter de Lyon (certifié Tier~III+), situé à plus de 250~km du site principal. Cette distance géographique garantit une indépendance vis-à-vis des risques régionaux (catastrophe naturelle, coupure réseau de collecte). Nous y prévoyons \textbf{1 baie 42U} avec 5~kW de puissance.
...
Nous proposons une architecture de virtualisation basée sur \textbf{VMware vSphere 8} (Standard Edition), qui constitue la référence du marché pour les environnements de production critiques. Ce choix se justifie par :

\begin{itemize}
\item La maturité et la fiabilité éprouvée de la plateforme en environnement de production ;
\item La compatibilité native avec les outils de sauvegarde (Veeam) et de réplication ;
\item La richesse fonctionnelle : vMotion (migration à chaud), DRS (répartition dynamique de charge), HA (haute disponibilité automatique) ;
...
\begin{itemize}
\item 256 c\oe urs physiques / 512 threads $\rightarrow$ largement dimensionné pour 330 vCPU ;
...
La capacité utile de 65~To couvre largement les 30~To de stockage SSD nécessaires pour les VM, avec une marge de croissance de plus de 100\%.
...
\begin{table}[H]
\centering
\begin{tabularx}{\textwidth}{L{2.5cm}C{2cm}C{2cm}C{2cm}C{2cm}}
\toprule
...
\centering
\begin{tikzpicture}[
  serveur/.style={draw, thick, fill=thbleu!10, minimum width=2cm, minimum height=0.6cm, font=\footnotesize, rounded corners},
  stockage/.style={draw, thick, fill=thorange!15, minimum width=2.5cm, minimum height=1.2cm, font=\footnotesize, align=center, rounded corners},
  baie/.style={draw, thick, dashed, rounded corners, inner sep=10pt},
  fleche/.style={-Stealth, thick},
]
  % DC Principal
  \node[font=\bfseries\small, thbleu] at (0,4.5) {DC Principal -- Toulouse};
  \node[serveur] (s1) at (-1.5,3.5) {Serveur 1};
  \node[serveur] (s2) at (-1.5,2.7) {Serveur 2};
  \node[serveur] (s3) at (-1.5,1.9) {Serveur 3};
  \node[serveur] (s4) at (-1.5,1.1) {Serveur 4};
  \node[stockage] (st1) at (1.5,2.3) {PowerStore\\1200T\\96 To SSD};
  \node[serveur] (as1) at (0,0) {AS/400 \#1};
  \node[serveur] (as2) at (2.5,0) {AS/400 \#2};
  \node[baie, fit=(s1)(s2)(s3)(s4)(st1)(as1)(as2), label=above:{}] (dcprin) {};

  % DC PRA
  \node[font=\bfseries\small, thbleu] at (9,4.5) {DC Secours -- Lyon};
  \node[serveur] (s5) at (7.5,3.5) {Serveur 5};
  \node[serveur] (s6) at (7.5,2.7) {Serveur 6};
  \node[stockage] (st2) at (10.5,3.1) {PowerStore\\500T\\46 To SSD};
  \node[baie, fit=(s5)(s6)(st2), label=above:{}] (dcpra) {};

  % Réplication
  \draw[fleche, red!70!black, dashed] (st1.east) -- node[above, font=\footnotesize] {Réplication SAN} node[below, font=\footnotesize] {asynchrone (4h)} (st2.west);
  \draw[fleche, thorange, dashed] ([yshift=-5pt]dcprin.east) -- node[below, font=\footnotesize, yshift=-3pt] {Veeam Replication} ([yshift=-5pt]dcpra.west);
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.85, transform shape,
  cloudnode/.style={draw, thick, fill=thgris, cloud, cloud puffs=12, cloud puff arc=120, minimum width=5cm, minimum height=2.5cm, align=center, font=\small\bfseries},
  dcbox/.style={draw, thick, fill=thbleu!10, rounded corners, minimum width=3.8cm, minimum height=1.8cm, align=center, font=\small},
  sitebox/.style={draw, thick, fill=thorange!10, rounded corners, minimum width=3cm, minimum height=1.5cm, align=center, font=\footnotesize},
  fwbox/.style={draw, thick, fill=red!10, rounded corners, minimum width=2cm, minimum height=0.8cm, align=center, font=\footnotesize},
]
  % Cloud MPLS
  \node[cloudnode] (mpls) at (6,0) {MPLS\\Orange Business};

  % Internet
  \node[cloudnode, minimum width=3.5cm, minimum height=2cm] (inet) at (13,0) {Internet};

  % DC Principal
  \node[dcbox] (dc1) at (0,4) {\textbf{DC Toulouse}\\4 serveurs + SAN\\2 AS/400};
  \node[fwbox] (fw1) at (0,1.5) {FortiGate\\600F (x2)};

  % DC PRA
  \node[dcbox] (dc2) at (13,4) {\textbf{DC Lyon}\\PRA};
  \node[fwbox] (fw2) at (13,1.5) {FortiGate\\200F};

  % Liens DC-MPLS
  \draw[-Stealth, thick, thbleu] (fw1.south) -- (fw1.south |- mpls.west) -- (mpls.west);
  \draw[-Stealth, thick, thbleu] (dc1.south) -- (fw1.north);
  \draw[-Stealth, thick, thbleu] (fw2.south) -- (fw2.south |- mpls.east) -- (mpls.east);
  \draw[-Stealth, thick, thbleu] (dc2.south) -- (fw2.north);

  % Lien Internet
  \draw[-Stealth, thick, thgrisfonce, dashed] (fw1.east) -- ++(1.5,0) |- (inet.west);
  \draw[-Stealth, thick, thgrisfonce, dashed] (fw2.west) -- ++(-0.5,0) -- (inet.north);

  % Sites
  \node[sitebox] (sA1) at (0,-3.5) {Centre stockage\\Bordeaux};
  \node[sitebox] (sA2) at (3,-3.5) {Centre stockage\\Nantes};
  \node[sitebox] (sB1) at (6,-3.5) {Centre tri\\Lille};
  \node[sitebox] (sB2) at (9,-3.5) {Centre tri\\Marseille};
  \node[sitebox] (sC1) at (12,-3.5) {Plateau bureau\\Paris (Siège)};

  \node[font=\footnotesize, thgrisfonce] at (6,-4.8) {\textit{... et 16 autres sites}};

  % SD-WAN sur sites
  \node[fwbox, minimum width=1.5cm] (fws1) at (0,-2) {FG-60F};
  \node[fwbox, minimum width=1.5cm] (fws2) at (3,-2) {FG-60F};
  \node[fwbox, minimum width=1.5cm] (fws3) at (6,-2) {FG-60F};
  \node[fwbox, minimum width=1.5cm] (fws4) at (9,-2) {FG-60F};
  \node[fwbox, minimum width=1.5cm] (fws5) at (12,-2) {FG-60F};

  \draw[thick, thbleu] (mpls.south) -- ++(0,-0.5) -| (fws1.north);
  \draw[thick, thbleu] (mpls.south) -- ++(0,-0.5) -| (fws2.north);
  \draw[thick, thbleu] (mpls.south) -- ++(0,-0.5) -| (fws3.north);
  \draw[thick, thbleu] (mpls.south) -- ++(0,-0.5) -| (fws4.north);
  \draw[thick, thbleu] (mpls.south) -- ++(0,-0.5) -| (fws5.north);

  \draw[thick, thorange] (fws1.south) -- (sA1.north);
  \draw[thick, thorange] (fws2.south) -- (sA2.north);
  \draw[thick, thorange] (fws3.south) -- (sB1.north);
  \draw[thick, thorange] (fws4.south) -- (sB2.north);
  \draw[thick, thorange] (fws5.south) -- (sC1.north);
\end{tikzpicture}
...
La vidéosurveillance de BOOM représente un flux réseau significatif (jusqu'à 200 caméras par centre de stockage) avec des contraintes spécifiques que nous adressons comme suit :
...
Les environnements d'entrepôt présentent des défis majeurs pour le WiFi : structures métalliques des rayonnages qui créent des réflexions et des zones d'ombre, températures extrêmes dans les zones froid négatif ($-25$°C) et froid positif ($+2$°C), et forte densité de terminaux mobiles se déplaçant rapidement (chariots à 10-15 km/h).
...
\item Alimentation PoE+ (802.3at) -- compatible avec les switches Cisco Catalyst 9200/9300 ;
\item Température de fonctionnement de $-20$°C à $+50$°C pour le modèle industriel (C9130AXI), adapté aux zones de froid positif.
\end{itemize}
...
\item La richesse fonctionnelle native : standard automatique, files d'attente, enregistrement d'appels, intégration CRM, visioconférence intégrée ;
...
\centering
\small
\begin{tabularx}{\textwidth}{L{3cm}L{3.5cm}C{1.5cm}L{4.5cm}}
\toprule
...
La supervision est assurée par \textbf{Zabbix} (solution open-source éprouvée), déployé sur une VM dédiée dans le datacenter principal. Zabbix collecte en temps réel les métriques de l'ensemble des composants de l'infrastructure :
...
\begin{ganttchart}[
  hgrid,
  vgrid={*{11}{draw=thgris}},
  x unit=1cm,
  y unit title=0.6cm,
  y unit chart=0.55cm,
  title height=1,
  bar height=0.6,
  bar/.append style={fill=thbleu!60},
  bar incomplete/.append style={fill=thorange!40},
  milestone/.append style={fill=thorange},
  group/.append style={draw=thbleu, fill=thbleu!20},
  ...
  \ganttgroup{Déploiement sites}{5}{10} \\
  ...
  \ganttmilestone{Mise en exploitation}{12}
\end{ganttchart}
...
\centering
\small
\begin{tabularx}{\textwidth}{L{5cm}C{1.2cm}R{2cm}R{2.5cm}}
\toprule
...
\centering
\small
\begin{tabularx}{\textwidth}{L{5.5cm}C{1cm}R{1.8cm}R{2.5cm}}
\toprule
\textbf{Désignation} & \textbf{Qté} & \textbf{PU HT} & \textbf{Total HT} \\
\midrule
...
\centering
\small
\begin{tabularx}{\textwidth}{L{5.5cm}C{1.5cm}R{1.5cm}R{2.5cm}}
\toprule
...
\centering
\small
\begin{tabularx}{\textwidth}{L{6cm}C{1.2cm}R{1.5cm}R{2.5cm}}
\toprule
...
Ce ratio est cohérent avec les standards du marché pour une infrastructure complète (poste de travail + réseau + téléphonie + datacenter) dans le secteur de la logistique industrielle, où les contraintes de disponibilité et de robustesse des équipements (terminaux durcis, WiFi entrepôt, DECT) tirent les coûts vers le haut par rapport à un environnement tertiaire classique.
...
\begin{itemize}
\item La présente offre est valable \textbf{90 jours} à compter de sa date d'émission (15 novembre 2024).
...
\item Les délais de livraison des équipements sont donnés à titre indicatif et peuvent varier en fonction de la disponibilité des constructeurs au moment de la commande.
